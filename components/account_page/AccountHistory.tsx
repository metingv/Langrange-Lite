...
// @ts-ignore
  ...
  const exportHistoryToCSV = () => {
    ...
    if (view == 'Trades') {
      dataToExport = tradeHistory.map((trade:any) => {
      ...
      dataToExport = history
        .filter((val:any) => val.activity_type == view)
        .map((row:any) => {
    ...
    const tab = historyViews.filter((v:any) => v.key == view)[0].label
...
const parseActivityDetails = (activity_details: { asset_symbol: any; liab_type: string; liab_symbol: string | string[]; liab_amount: any; asset_amount: string; asset_price: string; liab_price: string; asset_type: string }, activity_type: string, perpMarket: PerpMarket) => {
...
const LiquidationHistoryTable = ({ history, view }) => {
  ...
  const filteredHistory = useMemo(() => {
    return history?.length
      ? history.filter((h:any) => h.activity_type.includes('liquidate'))
      : []
  }, [history, view])
...
const HistoryTable = ({ history, view }) => {
  ...
  const filteredHistory = useMemo(() => {
    return history?.length
      ? history
          .filter((h: { activity_type: any }) => h.activity_type === view)
          .map((h: { activity_details: any }) => h.activity_details)
      : []
  }, [history, view])
  const { items, requestSort, sortConfig } = useSortableData(filteredHistory)
