...
export const handleDustTicks = (v: number | bigint) =>
// @ts-ignore
...
const handleUsdDustTicks = (v: number | bigint) =>// @ts-ignore
  Math.abs(v) < 0.0000099
    ? v === 0
      ? '$0'// @ts-ignore
      : `$${v.toExponential()}`
...
const AccountInterest = () => {
  ...
  const mangoAccountPk = useMemo(() => {// @ts-ignore
  ...
  const exportInterestDataToCSV = () => {
    ...
    for (const asset of assets) {
      dataToExport = [// @ts-ignore
        ...dataToExport,
        ...hourlyInterestStats[asset].map((interest: { time: string | number | Date; deposit_interest: any; borrow_interest: any }) => {
...
// @ts-ignore
  ...
  useEffect(() => {
    ...
    const fetchInterestStats = async () => {
      ...
      if (hideInterestDust) {
        Object.entries(parsedResponse).forEach((r) => {
          ...
          const token = tokens.find((t) => t.symbol === r[0])// @ts-ignore
    ...
    const fetchHourlyInterestStats = async () => {
      ...
      let assets
      if (hideInterestDust) {
        const assetsToShow = hideDust.map((a: any[]) => a[0])
...
// @ts-ignore
        stats[asset] = x// @ts-ignore
          ...
          .filter((x: any) => x)
          .reverse()// @ts-ignore
        if (stats[asset].length === 0) {// @ts-ignore
          delete stats[asset]
        }
      }
  ...
  useEffect(() => {
    if (hourlyInterestStats[selectedAsset]) {
      ...
      const filtered = hourlyInterestStats[selectedAsset].filter(
        (d: { time: string | number | Date }) => new Date(d.time).getTime() > start
      )

      const dailyInterest: never[] = []

      for (let i = 0; i < 30; i++) {
        dailyInterest.push({// @ts-ignore
          interest: 0,// @ts-ignore
          value: 0,// @ts-ignore
          time: new Date(
            // @ts-ignore
      ...
      filtered.forEach((d: { time: string | number | Date | dayjs.Dayjs | null | undefined; borrow_interest: number; deposit_interest: number; price: number }) => {
        ...
        if (found) {
          ...
          const newValue =
            d.borrow_interest > 0
              ...
              : d.deposit_interest * d.price// @ts-ignore
          found.interest = found.interest + newInterest// @ts-ignore
            ...
            {!isEmpty(hourlyInterestStats) && !loadHourlyStats ? (
              <>
                <div className="flex items-center justify-between pb-4 pt-8 w-full">
                  ...
                  <Select
                    value={selectedAsset}
                    onChange={(a: SetStateAction<string>) => setSelectedAsset(a)}
                ...
                {selectedAsset && chartData.length > 0 ? (
                  <div className="flex flex-col sm:flex-row space-x-0 sm:space-x-4 w-full">
                    {
                    chartData.find((d:any) => d.interest !== 0) ? (
                      <div
                    ...
                    {chartData.find((d:any) => d.value !== 0) ? (
                      <div
                        className="border border-th-bkg-4 relative mb-6 p-4 rounded-md w-full sm:w-1/2"
                        style={{ height: '330px' }}
                      >
                        <Chart
                          ...
                          labelFormat={(x) =>
                            x === 0
                              ? 0
                              : x < 0// @ts-ignore
                              ? `-$${Math.abs(x)?.toFixed(token.decimals + 1)}`// @ts-ignore
                  ...
                  <div>
                    {paginatedData.length ? (
                      <Table>
                        ...
                        <tbody>
                          {paginatedData.map((stat: { time: Key | null | undefined; borrow_interest: number; deposit_interest: number; price: number }) => {
                            // @ts-ignore
                              ...
                              <TrBody key={stat.time}>
                                ...
                                <Td className="w-1/3">
                                  {stat.borrow_interest > 0
                                    ? `-${stat.borrow_interest.toFixed(// @ts-ignore
                                        token.decimals + 1
                                      )}`
                                    : stat.deposit_interest.toFixed(// @ts-ignore
                                        token.decimals + 1
                                      )}{' '}
                                  {selectedAsset}
                                </Td>
                                <Td className="w-1/3">
                                  {stat.borrow_interest > 0
                                    ? `-$${(
                                        stat.borrow_interest * stat.price// @ts-ignore
                                      ).toFixed(token.decimals + 1)}`
                                    : `$${(
                                        stat.deposit_interest * stat.price// @ts-ignore
                                      ).toFixed(token.decimals + 1)}`}
                                </Td>
                              </TrBody>
                            )
