...
const AccountFunding = () => {
  ...
  const exportFundingDataToCSV = () => {
    ...
    for (const asset of assets) {
      dataToExport = [
        ...dataToExport,
        ...hourlyFunding[asset].map((funding: { time: string | number | Date; total_funding: any }) => {
...
// @ts-ignore
  ...
  useEffect(() => {
    if (hourlyFunding[selectedAsset]) {
      ...
      const dailyFunding: number  | never[] = []

      for (let i = 0; i < 30; i++) {
        dailyFunding.push({// @ts-ignore
          funding: 0,// @ts-ignore
          time: new Date(
            // @ts-ignore
      ...
      filtered.forEach((d: { time: string | number | Date | dayjs.Dayjs | null | undefined; total_funding: any }) => {
        const found = dailyFunding.find(
          (x:any) =>
            dayjs(x.time).format('DD-MMM') === dayjs(d.time).format('DD-MMM')
        )
        if (found) {
          const newFunding = d.total_funding// @ts-ignore
              ...
              <>
                ...
                {selectedAsset && chartData.length > 0 ? (
                  <div className="flex flex-col sm:flex-row space-x-0 sm:space-x-4 w-full">
                    {chartData.find((d:any) => d.funding !== 0) ? (
                      <div
