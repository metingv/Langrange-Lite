    ...
    try {
      ...
      if (market instanceof Market) {
        txid = await mangoClient.placeSpotOrder(
          mangoGroup,
          mangoAccount,
          mangoGroup.mangoCache,// @ts-ignore
          market,
          wallet,
          side,
          orderPrice,
            ...
            {!hideProfitStop ? (
              ...
              >
                <Checkbox
                  ...
                  onChange={(e: { target: { checked: boolean | ((prevState: boolean) => boolean) } }) => setShowStopForm(e.target.checked)}
                >
                  {t('set-stop-loss')}
                </Checkbox>
              </div>
            ...
            {!hideProfitStop ? (
              ...
              >
                <Checkbox
                  ...
                  onChange={(e: { target: { checked: boolean | ((prevState: boolean) => boolean) } }) => setShowTakeProfitForm(e.target.checked)}
                >
                  {t('set-take-profit')}
                </Checkbox>
              </div>
        ...
        <div className="col-span-12 flex pt-2">
              ...
              <div className="mr-4">
                ...
                >
                  <div className="flex items-center text-th-fgd-3 text-xs">
                    <Checkbox
                      checked={ioc}
                      onChange={(e: { target: { checked: any } }) => iocOnChange(e.target.checked)}
                    >
                      IOC
                    </Checkbox>
                  </div>
          ...
          {marketConfig.kind === 'perp' ? (
            <Tooltip delay={250} placement="left" content={t('tooltip-reduce')}>
              <Checkbox
                checked={reduceOnly}
                onChange={(e: { target: { checked: any } }) => reduceOnChange(e.target.checked)}
              >
                Reduce Only
              </Checkbox>
            </Tooltip>
          ...
          {marketConfig.kind === 'spot' ? (
            ...
            >
              <Checkbox
                checked={spotMargin}
                onChange={(e: { target: { checked: boolean | ((prevState: boolean) => boolean) } }) => setSpotMargin(e.target.checked)}
              >
                Margin
              </Checkbox>
            </Tooltip>
