...
// @ts-ignore
  ...
  const [maxSlippagePercentage, setMaxSlippagePercentage] = useState(// @ts-ignore
  ...
  const saveMaxSlippage = (slippage: number) => {
  ...
  useEffect(
    () =>
      useMangoStore.subscribe(
        // @ts-ignore
        (orderBook) => (orderBookRef.current = orderBook),// @ts-ignore
  ...
  const setBaseSize = (baseSize: string | number) =>
    set((s) => {// @ts-ignore
      if (!Number.isNaN(parseFloat(baseSize))) {// @ts-ignore
  ...
  const setQuoteSize = (quoteSize: string) =>
    set((s) => {
  ...
  const setTradeType = (type: string) => {
    set((s) => {// @ts-ignore
      s.tradeForm.tradeType = type
    })
  }

  const setTriggerPrice = (price: string) => {
    set((s) => {
      if (!Number.isNaN(parseFloat(price))) {
...
// @ts-ignore
  ...
  useEffect(
    () =>
      useMangoStore.subscribe(
        (markPrice) => (markPriceRef.current = markPrice as number),// @ts-ignore
  ...
  const onSetPrice = (price: number | '') => {// @ts-ignore
    setPrice(price)
    if (!price) return
    if (baseSize) {
  ...
  const onSetBaseSize = (baseSize: number | '') => {// @ts-ignore
  ...
  const onSetQuoteSize = (quoteSize: number | '') => {// @ts-ignore
...
// @ts-ignore
  ...
  const iocOnChange = (checked: boolean | ((prevState: boolean) => boolean)) => {
    if (checked) {
      setPostOnly(false)
      setPostOnlySlide(false)
    }
  ...
  const reduceOnChange = (checked: boolean | ((prevState: boolean) => boolean)) => {
    handleSetPositionSize(positionSizePercent, spotMargin, checked)
    setReduceOnly(checked)
  }
  const marginOnChange = (checked: boolean | ((prevState: boolean) => boolean)) => {
  ...
  const handleSetPositionSize = (percent: SetStateAction<string>, spotMargin: boolean | ((prevState: boolean) => boolean), reduceOnly: boolean | ((prevState: boolean) => boolean)) => {
  ...
  const percentToClose = (size: number, total: number) => {
    if (!size || !total) return 0
    return (size / total) * 100
  }
// @ts-ignore
  const roundedDeposits = parseFloat(deposits?.toFixed(sizeDecimalCount))// @ts-ignore
  const roundedBorrows = parseFloat(borrows?.toFixed(sizeDecimalCount))

  const closeDepositString =// @ts-ignore
  ...
  const closeBorrowString =// @ts-ignore
...
// @ts-ignore
    const mangoAccount = useMangoStore.getState().selectedMangoAccount.current// @ts-ignore
    const mangoGroup = useMangoStore.getState().selectedMangoGroup.current
    const askInfo =// @ts-ignore
      useMangoStore.getState().accountInfos[marketConfig.asksKey.toString()]
    const bidInfo =// @ts-ignore
      useMangoStore.getState().accountInfos[marketConfig.bidsKey.toString()]// @ts-ignore
    const wallet = useMangoStore.getState().wallet.current// @ts-ignore
    ...
    /*       null, */
      ...
      notify({
        title: t('order-error'),// @ts-ignore
        description: e.message,// @ts-ignore
        txid: e.txid,
        type: 'error',
      })
      console.error(e)
              ...
              <div className="flex">
                <div className="mr-4 mt-3">
                  ...
                  >
                    <Checkbox
                      checked={postOnly}
                      onChange={(e: { target: { checked: any } }) => postOnChange(e.target.checked)}
                    >
                      POST
                    </Checkbox>
                  </Tooltip>
                ...
                <div className="mr-4 mt-3">
                  ...
                  >
                    <div className="flex items-center text-th-fgd-3 text-xs">
                      <Checkbox
                        checked={ioc}
                        onChange={(e: { target: { checked: boolean | ((prevState: boolean) => boolean) } }) => iocOnChange(e.target.checked)}
                      >
                        IOC
                      </Checkbox>
                    </div>
            ...
            {marketConfig.kind === 'perp' ? (
              <div className="mt-3">
                ...
                >
                  <Checkbox
                    checked={reduceOnly}
                    onChange={(e: { target: { checked: boolean | ((prevState: boolean) => boolean) } }) => reduceOnChange(e.target.checked)}
            ...
            {marketConfig.kind === 'spot' ? (
              <div className="mt-3">
                ...
                >
                  <Checkbox
                    checked={spotMargin}
                    onChange={(e: { target: { checked: boolean | ((prevState: boolean) => boolean) } }) => marginOnChange(e.target.checked)}
                  >
                    {t('margin')}
                  </Checkbox>
                </Tooltip>
