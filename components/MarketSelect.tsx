...
const MarketSelect = () => {
  ...
  useEffect(() => {
    const markets: any = []
// @ts-ignore
    const allMarkets: any  = [...groupConfig.spotMarkets, ...groupConfig.perpMarkets]
    allMarkets.forEach((market: any ) => {
      const base = market.name.slice(0, -5)
      const found = markets.find((b: any ) => b.baseAsset === base)
      if (!found) {
      ...
      <div className="bg-th-bkg-3 flex h-10 w-full">
        ...
        <div className="flex items-center justify-between w-full">
          <div className="flex items-center">
            {sortedMarkets
            // @ts-ignore
              .filter((m:any) => !hiddenMarkets.includes(m.baseAsset))
              .map((s) => (
                <MarketMenuItem
                // @ts-ignore
                  key={s.baseAsset}// @ts-ignore
                  linksArray={s.markets}// @ts-ignore
