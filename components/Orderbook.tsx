...
const Line = (props: { [x: string]: any; className: any; invert: any; }) => {
  return (
    <div
...
const groupBy = (ordersArray: string | any[], market: Market | PerpMarket | null, grouping: number, isBids: boolean) => {
...
const getCumulativeOrderbookSide = (
  ...
  let cumulative = orders
    .slice(0, depth)
    .reduce((cumulative: { cumulativeSize: any; }[], [price, size]: any, i: number) => {
...
const hasOpenOrderForPriceGroup = (openOrderPrices, price, grouping) => {
  ...
  return !!openOrderPrices.find((ooPrice: number) => {
...
export default function Orderbook({ depth = 8 }) {
  ...
  useInterval(() => {
    if (
      ...
      !isEqualLodash(
        // @ts-ignore
        currentOrderbookData.current.bids,// @ts-ignore
        nextOrderbookData.current.bids
      ) ||
      !isEqualLodash(// @ts-ignore
        currentOrderbookData.current.asks,// @ts-ignore
        nextOrderbookData.current.asks// @ts-ignore
      ) ||
      previousDepth !== depth ||
      previousGrouping !== grouping
    ) {
      // check if user has open orders so we can highlight them on orderbook
      const openOrders =// @ts-ignore
...
// @ts-ignore
      const sum = (total, [, size], index) =>
        index < depth ? total + size : total// @ts-ignore
      const totalSize = bids.reduce(sum, 0) + asks.reduce(sum, 0)
      const maxSize =
        Math.max(// @ts-ignore
          ...asks.map(function (a:any) {
            return a[1]
          })
        ) +
        Math.max(// @ts-ignore
          ...bids.map(function (b:any) {
            return b[1]
          })
        )
...
// @ts-ignore
  ...
  const handleLayoutChange = () => {
    ...
    setOrderbookData((prevState) => ({// @ts-ignore
      ...orderbookData,// @ts-ignore
      asks: prevState.asks.reverse(),
    }))
  }

  const onGroupSizeChange = (groupSize:any) => {
  ...
  return !isMobile ? (
                ...
                <div className="w-1/2">
                  ...
                  orderbookData?.bids.map(
                    ({// @ts-ignore
                      price,// @ts-ignore
                      size,// @ts-ignore
                      cumulativeSize,// @ts-ignore
                      sizePercent,// @ts-ignore
                      maxSizePercent,
                    }) => (
                      <OrderbookRow
                        market={market}
    ...
    <div>
      ...
      {
      // @ts-ignore
      orderbookData?.asks.map(// @ts-ignore
      ...
      {
      // @ts-ignore
      orderbookData?.bids.map(// @ts-ignore
        ({ price, size, cumulativeSize, sizePercent, maxSizePercent }) => (
          <OrderbookRow
            market={market}
            hasOpenOrder={hasOpenOrderForPriceGroup(
...
const OrderbookRow = React.memo<any>(
  ({
    ...
    useEffect(() => {
      showOrderbookFlash &&// @ts-ignore
        !element.current?.classList.contains(flashClassName) &&// @ts-ignore
      ...
      >
        {invert ? (
          ...
          <>
            ...
            <Line
                className={`absolute right-0 ${side === 'buy' ? `bg-th-green-muted` : `bg-th-red-muted`}`}
                data-width={sizePercent + '%'} invert={undefined}            />
          </>
        )}
      </div>
    )
