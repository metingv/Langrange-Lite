...
function formatNumberString(x: number, decimals: number): string {
...
export default function StatsTotals({ latestStats, stats }) {
  ...
  const formatValues = (values: any[]) => {
    ...
    const hours = values.reduce((acc, d) => {
      const found = acc.find((a: { time: any; name: any }) => a.time === d.time && a.name === d.name)
      const value = {
        value: d.value,
        name: d.name,
        time: d.time,
    ...
    hours.forEach(function (d: { time: string; value: any }) {
      if (d.time in holder) {// @ts-ignore
    ...
    for (const prop in holder) {// @ts-ignore
      ...
      {!isMobile ? (
          ...
          <div className="pb-8">
            ...
            {latestStats.length > 0 ? (
              <Table>
                ...
                <tbody>
                  {latestStats.map((stat: { name: {} | null | undefined; totalDeposits: number; totalBorrows: number; depositInterest: { toNumber: () => number }; borrowInterest: { toNumber: () => number }; utilization: { mul: (arg0: I80F48) => { (): any; new(): any; toNumber: { (): number; new(): any } } } }) => (
                    // @ts-ignore
                    <TrBody key={stat.name}>
                      ...
                      <Td>
                        {formatNumberString(
                          stat.totalDeposits,// @ts-ignore
                          tokenPrecision[stat.name]
                        )}
                      </Td>
                      <Td>
                        {formatNumberString(
                          stat.totalBorrows,// @ts-ignore
                          tokenPrecision[stat.name]
                        )}
                      </Td>
                      <Td>
          ...
          <div className="pb-8">
            ...
            {stats.length > 1 ? (
              <Table>
                ...
                <tbody>
                  {latestStats.map((stat: { name: {} | null | undefined }) => (// @ts-ignore
                    <TrBody key={stat.name}>
                      <Td>
                        <div className="flex items-center">
                          <img
        ...
        <>
          <div className="pb-8">
            ...
            {latestStats.map((stat: { name: {} | null | undefined; totalDeposits: number; totalBorrows: number; depositInterest: { toNumber: () => number }; borrowInterest: { toNumber: () => number }; utilization: { mul: (arg0: I80F48) => { (): any; new(): any; toNumber: { (): number; new(): any } } } }, index: number) => (
              // latestStats.length > 0 ? (
              <ExpandableRow
                buttonTemplate={
                    ...
                    <div className="col-span-12 sm:col-span-6 flex items-center text-fgd-1">
                      <div className="flex items-center">
                        <img
                          ...
                          width="20"
                          height="20"// @ts-ignore
                          src={`/assets/icons/${stat.name// @ts-ignore
                            .split(/-|\//)[0]
                            .toLowerCase()}.svg`}
                          className={`mr-2.5`}
                        />
          ...
          <div className="pb-8">
            ...
            {stats.length > 1
              ? latestStats.map((stat: { name: {} | null | undefined }, index: number) => (// @ts-ignore
          ...
          {stats.length > 1
            ? latestStats.map((stat: { name: {} | null | undefined }, index: number) => (
              // @ts-ignore
                <Row key={stat.name} index={index}>
                  <div className="grid grid-cols-12 grid-rows-2 sm:grid-rows-1 gap-2 text-left sm:text-right">
                    <div className="col-span-12 sm:col-span-3 flex items-center text-fgd-1">
