...
function calculateFundingRate(
  oldestLongFunding: string,
  oldestShortFunding: string,
  latestLongFunding: string,
  latestShortFunding: string,
  perpMarket: PerpMarket | undefined,
  oraclePrice: number
) {
...
export default function StatsPerps({ perpStats }) {
  ...
  const [selectedAsset, setSelectedAsset] = useState<string>('BTC-PERP')// @ts-ignore
  ...
  const selectedMarket = useMemo(() => {
    return perpMarkets.find((m) =>// @ts-ignore
  ...
  const perpsData = useMemo(() => {
    ...
    let selectedStatsData = perpStats.filter(
      (stat: { name: string }) => stat.name === selectedAsset
    )

    if (selectedAsset == 'SOL-PERP') {
      const startTimestamp = 1632160800000
      selectedStatsData = selectedStatsData.filter(
        (stat: { hourly: string | number | Date }) => new Date(stat.hourly).getTime() >= startTimestamp
      )
    }

    const perpsData = selectedStatsData.map((x: { oldestLongFunding: string; oldestShortFunding: string; latestLongFunding: string; latestShortFunding: string; baseOraclePrice: number; openInterest: BN; hourly: any }) => {
      return {
    ...
    if (selectedAsset === 'BTC-PERP') {
      const index = perpsData.findIndex(
        (x: { time: string }) => x.time === '2021-09-15T05:00:00.000Z'
      )
