...
const TVChartContainer = () => {
  ...
  const handleCancelOrder = async (
    ...
    const selectedMangoGroup =// @ts-ignore
      useMangoStore.getState().selectedMangoGroup.current
    const selectedMangoAccount =// @ts-ignore
      useMangoStore.getState().selectedMangoAccount.current// @ts-ignore
    const mangoClient = useMangoStore.getState().connection.client
    let txid
    try {
      ...
      } else if (market instanceof PerpMarket) {// @ts-ignore
  ...
  const handleModifyOrder = async (
    ...
    const mangoGroup = useMangoStore.getState().selectedMangoGroup.current// @ts-ignore
...
// @ts-ignore
  function getLine(order, market) {// @ts-ignore
    return tvWidgetRef.current
      ...
      .onMove(function () {
        ...
        const currentOrderPrice = order.price// @ts-ignore
      ...
      .onCancel(function () {
        toggleOrderInProgress(true)// @ts-ignore
        tvWidgetRef.current.showConfirmDialog({
  ...
  function getLineText(order: { side: string; perpTrigger: { clientOrderId: any; triggerPrice: number; triggerCondition: string }; orderType: string }, market: { config: { baseDecimals: number; quoteDecimals: number; baseSymbol: any } }) {
  ...
  function deleteLines() {// @ts-ignore
  ...
  function drawLines() {
    const tempLines = new Map()// @ts-ignore
    tvWidgetRef.current.onChartReady(() => {
