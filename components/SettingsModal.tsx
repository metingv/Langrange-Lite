...
const DefaultMarketSettings = ({ setSettingsView }) => {
  ...
  const allMarkets = [// @ts-ignore
    ...groupConfig.spotMarkets,// @ts-ignore
  ...
  const handleSetDefaultMarket = (market: string | string[]) => {
    const base = market.slice(0, -5)
    const kind = market.includes('PERP') ? 'perp' : 'spot'

    setDefaultMarket({// @ts-ignore
      base: base,
      kind: kind,// @ts-ignore
      name: market,
      path: `/?name=${market}`,
  ...
  return (
    <div>
      ...
      <Select
        ...
        onChange={(market: string | string[]) => handleSetDefaultMarket(market)}
        className="w-full"
      >
...
const RpcEndpointSettings = ({ setSettingsView }) => {
  ...
  const handleSetEndpointUrl = (endpointUrl: string) => {
  ...
  return (
    <div className="flex flex-col text-th-fgd-1">
      ...
      // @ts-ignore
        ...
        onChange={(url: string) => handleSelectEndpointUrl(url)}
        className="w-full"
      >
